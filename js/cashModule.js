...
let cashData = {};           // Кэш данных о денежных средствах
let accountsVisible = false; // Флаг: показаны ли все счета

// ========================
// ОСНОВНОЙ РАСЧЕТ ДЕНЕЖНЫХ СРЕДСТВ
// ========================
...
function calculateCashData() {
    ...
    const initialBalances = {};     // Начальные остатки по счетам
    const allAccounts = new Set();    // Все уникальные счета
    ...
    // ========================
    const cumulativeIncome = {};    // Накопленные доходы
    const cumulativeExpense = {};   // Накопленные расходы
    const allTransactions = {};      // Все операции для деталей
...
// ========================
// ОТРИСОВКА БЛОКА ДЕНЕЖНЫХ СРЕДСТВ
// ========================
...
function calculateAndRenderDeposit() {
    ...
    if (depositSidebarBlock && depositTotalValue) {
        if (depositBalance !== 0 || depositOperations.length > 0) {
            ...
            depositTotalValue.innerHTML = formatCash(depositBalance, 'RUB');
            ...
            const operationsHtml = depositOperations.slice(-15).map(op => `
                <div class="cash-detail-row">
                    ...
                    <span style="display: flex; gap: 8px;">
                        ...
                        <span class="${op.sign === '+' ? 'cash-income' : 'cash-expense'}">${op.sign === '+' ? '+' : '-'}${formatCash(op.amount, 'RUB')}</span>
                    </span>
                </div>
            `).join('');
