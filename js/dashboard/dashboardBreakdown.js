...
function generateFullAverageCheckBreakdown(data) {
    ...
    <div style="margin-bottom: 16px; padding: 12px; background: rgba(102,126,234,0.1); border-radius: 12px;">
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
            <div><div style="font-size: 12px;">Общий средний чек</div><div style="font-size: 24px; font-weight: 700;">${formatCurrency(overallAvgCheck)}</div></div>
            <div><div style="font-size: 12px;">🏆 Лидер по чеку</div><div style="font-size: 16px; font-weight: 600;">${topChannel.name} <span style="font-size: 14px; color: #667eea;">${formatCurrency(topChannel.avgCheck)}</span></div></div>
            <div><div style="font-size: 12px;">📈 Прогноз на след. месяц</div><div style="font-size: 16px; font-weight: 600; color: #48bb78;">${formatCurrency(forecastAvgCheck)}</div><div style="font-size: 10px;">+5% оптимистичный</div></div>
        </div>
    </div>`;
...
console.log('✅ dashboardBreakdown.js: загружен');
